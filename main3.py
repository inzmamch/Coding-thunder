...
app.config['UPLOAD_FILE'] = params['upload_loc']
app.config.update(
    MAIL_SERVER = 'smtp.gmail.com',
    MAIL_PORT = '465',
    MAIL_USE_SSL = True,
    MAIL_USERNAME = params['gmail_user'],
    MAIL_PASSWORD = params['gmail_pasw']
)
mail = Mail(app)
if local_server:
    app.config['SQLALCHEMY_DATABASE_URI'] = params['loc_url']
else:
...
def contact():
    if (request.method == 'POST'):
        ...
        db.session.commit()
        mail.send_message('New message from '+ name, sender = email, recepients = [params['gmail_user']], body = message + "\n" + phone)
